...
class report_overviewstats_chart_courses extends report_overviewstats_chart {
	...
	public function get_content() {
		...
		return array($title => array(
			...
			$titlesizes => html_writer::tag('div', $this->get_course_chart(), array(
	 ...
	 * @return chart html
	 */
	protected function get_course_chart() {
		global $OUTPUT;
		$sales = new \core\chart_series('Nuber of courses', $this->data['sizes']['courses']);
		$labels = $this->data['sizes']['course_size'];
	...
	protected function prepare_data() {
		...
		foreach ($data as $distributiongroup => $courses) {
			...
			$this->data['sizes']['course_size'][] = $distributiongroupname;
			$this->data['sizes']['courses'][] = $courses;
		}
	}
}
